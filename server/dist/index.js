...
function buildFallbackResults(answers, context) {
    const scores = {
        communication: 0,
        availability: 0,
        habits: 0,
    ...
    const fallbackInsights = Object.keys(scores)
        .map((category) => ({
        category,
...
function isGeneratedResults(value) {
    ...
    return typeof candidate.headline === 'string'
        ...
        && candidate.insights.every((insight) => (insight
...
async function generateAiResults(answers, context) {
...
app.post('/api/generate-results', async (req, res) => {
    ...
    try {
        ...
        await db.run('INSERT INTO results (context, answers, insights) VALUES (?, ?, ?)', [JSON.stringify(context ?? null), JSON.stringify(answers ?? {}), JSON.stringify(result)]);
        res.json({ result });
    }
    catch (error) {
        console.error('[/api/generate-results] error:', error);
...
app.post('/api/chat', async (req, res) => {
    try {
        const { messages, context, answers, results, } = req.body;
        const apiKey = (process.env.DEEPSEEK_API_KEY || process.env.VITE_DEEPSEEK_API_KEY)?.trim();
        if (!apiKey) {
